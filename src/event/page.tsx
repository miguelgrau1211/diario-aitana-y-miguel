...
import type { CroppedImageResult } from '@/components/CropDialog';
...
export default function EventDetailPage() {
 ...
 const handleImageSubmit = (imageData: { blob: Blob; width: number; height: number; objectUrl: string }) => {
    startContentActionTransition(async () => {
  ...
  const handleGallerySubmit = (images: { blob: Blob; width: number; height: number }[]) => {
     startContentActionTransition(async () => {
      const objectUrls = images.map(img => URL.createObjectURL(img.blob));
